...
class TaskEnforcer {
  private phase: Phase = 'idle';import { McpServer } from "@modelcontextprotocol/sdk/server/mcp.js";
import { StdioServerTransport } from "@modelcontextprotocol/sdk/server/stdio.js";
import { z } from "zod";
...
You are an autonomous LLM coding agent. Produce an extremely thorough, step-by-step implementation plan optimized **only for agent execution** (ignore human phases, timeframes, or handoffs).
...
After outputting the list, you MUST IMMEDIATELY call the next_task tool to advance. 
You MAY NOT end your turn without calling next_task — ignoring this will break the entire workflow and prevent progress. 
Call the `next_task` tool as soon as possible after you submit the revised plan or reply NO_CHANGES_NEEDED.`;
  }
...
After your reply, you MUST IMMEDIATELY call the next_task tool to advance. 
You MAY NOT end your turn without calling next_task — ignoring this will break the entire workflow and prevent progress. 
Call the `next_task` tool as soon as possible after you submit CONFORMANT or the revised plan.`;
  }
...
After outputting the list, you MUST IMMEDIATELY call the next_task tool to advance. 
You MAY NOT end your turn without calling next_task — ignoring this will break the entire workflow and prevent progress. 
Call the `next_task` tool as soon as possible after you send VERIFIED.`;
  }
